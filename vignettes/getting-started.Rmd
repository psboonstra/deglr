...
$\lambda=0.01$, and $\lambda$ very large. The first corresponds to no pooling
...
This should give equivalent results to $\lambda$ very large, i.e. full pooling across populations. 
```{r}
glm_pool <- 
...
This should give equivalent results to $\lambda$ very large, i.e. full pooling
...
As we note above, the one exception to this is that when the outcome is a
time-to-even, the fully pooled Cox model is not actually fully pooled: separate
baseline hazards are still estimated. This explains why the point estimates are
